...
int total_second, w, treasure_count;
vector<pair<int, int> > treasure, choosen, res;
int DP[32][1002], flag=0;

int getMaxGold(int pos, int used_seconds){
    if(pos == treasure_count) return 0;
    if(DP[pos][used_seconds] != -1) return DP[pos][used_seconds];
    int cost = (3*w*treasure[pos].ff), ret=0;
    if(used_seconds+cost <= total_second){
        ret = treasure[pos].ss + getMaxGold(pos+1, used_seconds+cost);
    }
    ret = max(ret, getMaxGold(pos+1, used_seconds));
    return DP[pos][used_seconds]=ret;
}

void getTreasure(int pos, int goldTaken, int maxGold, int secondTaken){
    if(goldTaken == maxGold and secondTaken <= total_second){
        res = choosen;
        flag = 1;
        return;
    }
    if(pos==treasure_count) return;
    int cost = (3*w*treasure[pos].ff);
    if(secondTaken+cost <= total_second and goldTaken+treasure[pos].ss <= maxGold){
        choosen.push_back(treasure[pos]);
        getTreasure(pos+1, goldTaken+treasure[pos].ss, maxGold, secondTaken+cost);
        if(flag) return;
        choosen.pop_back();
    }
    getTreasure(pos+1, goldTaken, maxGold, secondTaken);
}
...
void solve(lli tc){
    ...
    while(cin >> total_second){
        if(!first) nl;
        cin >> w >> treasure_count;
        treasure.clear();
        choosen.clear();
        for(int i=0; i<treasure_count; i++){
            int t, n;
            cin >> t >> n;
            treasure.push_back({t, n});
        }
        mem(DP, -1);
        flag=0;
        int maxGold = getMaxGold(0, 0);
        print(maxGold);
        getTreasure(0, 0, maxGold, 0);
        print(res.sz)
        for(auto p : res){
            print(p.ff << " " << p.ss);
        }
