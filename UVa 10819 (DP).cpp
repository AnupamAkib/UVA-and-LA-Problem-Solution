...
int m, budget, price[101], value[101];
int DP[101][10300];

int getMaximumValue(int pos, int spendPrice, int budget_now, bool offerTaken){
    if(spendPrice > budget_now and offerTaken) return -inf;
    if(pos == m){
        if(spendPrice > budget_now){
            return -spendPrice;
        }
        return 0;
    }
    if(DP[pos][spendPrice] != -1) return DP[pos][spendPrice];
    int mx = -inf;
    bool OfferTakenFlag = offerTaken;
    int newSpend = spendPrice+price[pos];
    int newBudget = budget_now;
    if(newSpend > 2000 and OfferTakenFlag==false){
        newBudget = budget_now + 200;
        OfferTakenFlag = true;
    }
    mx = max(mx, value[pos] + getMaximumValue(pos+1, newSpend, newBudget, OfferTakenFlag));
    mx = max(mx, getMaximumValue(pos+1, spendPrice, budget_now, offerTaken));
    return DP[pos][spendPrice]=mx;
}
...
void solve(lli tc){
    ...
    mem(DP, -1);
    print(getMaximumValue(0, 0, budget, false));
}
