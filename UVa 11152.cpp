...
void solve(int tc){
    double a, b, c;
    while(cin >> a >> b >> c){
        double k = (a*b*c)/sqrt((a+b+c)*(b+c-a)*(c+a-b)*(a+b-c));
        double area_of_bigCircle = PI*k*k;
        double s = (a+b+c)/2.0;
        double area_of_Triangle = sqrt(s*(s-a)*(s-b)*(s-c));
        double r = area_of_Triangle/s;
        double area_of_smallCircle = PI*r*r;
        print(point(4) << area_of_bigCircle-area_of_Triangle << ' ' << area_of_Triangle-area_of_smallCircle << ' ' << area_of_smallCircle)
    }
}
