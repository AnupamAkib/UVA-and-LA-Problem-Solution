...
coor board, start, tmp, beeper[20];
int m, flag[20];

int dist(coor a, coor b){
    int d = abs(a.x-b.x)+abs(a.y-b.y);
    return d;
}

int backtrack(coor now, int c){
    if(c==m) return dist(now, start);
    int mini = inf, res = 0;
    for(int i=0; i<m; i++){
        if(!flag[i]){
            flag[i] = 1;
            res = dist(now, beeper[i])+backtrack(beeper[i], c+1);
            mini = min(mini, res);
            flag[i] = 0;
        }
    }
