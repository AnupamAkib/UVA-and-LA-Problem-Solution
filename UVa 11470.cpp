...
void solve(lli tc){
    ...
    for(int i=0; i<d; i++){
        ...
        for(int k=topLeft_y; k<=topRight_y; k++) sum += a[topRight_x][k];
        if(topLeft_x != topRight_x or topLeft_y != topRight_y){
            for(int k=downLeft_y; k<=downRight_y; k++) sum += a[downLeft_x][k];
        }
        topRight_x++;
        topLeft_x++;
        downLeft_x--;
        downRight_x--;
        for(int k=topLeft_x; k<=downLeft_x; k++) sum += a[k][topLeft_y];
        for(int k=topRight_x; k<=downRight_x; k++) sum += a[k][topRight_y];
        v.push_back(sum);
    }
    cout << "Case " << t++ << ": ";
    vprint(v);
