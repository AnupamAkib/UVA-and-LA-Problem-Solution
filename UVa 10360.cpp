...
void solve(int tc){
    int d = IN, x, y, population;
    int n = IN;
    vector<vector<int> > ar(1030, vector<int>(1030));
    for(int i=0; i<n; i++){
        x = IN, y = IN, population = IN;
        for(int p=max(0, x-d); p<=min(1025, x+d); p++){
            for(int q=max(0, y-d); q<=min(1025, y+d); q++){
                ar[p][q] += population;
            }
        }
    }
