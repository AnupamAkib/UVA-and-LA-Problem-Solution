...
//vertical & horizontal adjacent moves
int dx[] = {0, 0, -1, 1};
int dy[] = {1, -1, 0, 0};
...
void DFS(int i, int j){
    if(i<0 or i==mz.n or j<0 or j==mz.m) return;
    if(vis[i][j]) return;
    if(mz.maze[i][j] != ' ' and mz.maze[i][j] != '*') return;
    vis[i][j] = 1;
    mz.maze[i][j] = '#';
    for(int k=0; k<4; k++){
        DFS(i+dx[k], j+dy[k]);
