...
lli bigMod(lli n, lli p, lli m){
    if(p==0) return 1; //n^0 = 1, so returning 1 as base case
    if(p%2==0){ 
        lli r = bigMod(n, p/2, m); //even hole equal 2 vage vag kore dibo
        return ((r%m)*(r%m))%m; //law:  (a*b)%m = ((a%m)*(b%m))%m
        //ekhane a = b = r tai dui dikei r ache
    }
    else{
        return ((n%m)*(bigMod(n, p-1, m)%m))%m;
        //p odd tai p-1 kore even ta = b nilam r baki thake n. so, a=n dhorlam
    }
}
