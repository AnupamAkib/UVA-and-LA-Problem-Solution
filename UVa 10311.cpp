...
void solve(int tc){
    ...
    if(n&1==1){
        if(!mark[n-2]){
            print(n<<" is the sum of "<<2<<" and "<<n-2<<".")
        }
        else{
            print(n<<" is not the sum of two primes!")
        }
        ...
        int h = upper_bound(ALL(prime), n/2) - prime.begin() - 1, f =0;
        if(prime[h] == n/2) h--;
        for(int i=h; i>=0; i--){
            int p1 = prime[i];
            int p2 = n - p1;
            if(!mark[p2]){
                f = 1;
                print(n<<" is the sum of "<<p1<<" and "<<p2<<".")
                break;
            }
        }
        if(!f) print(n<<" is not the sum of two primes!")
    }
}
