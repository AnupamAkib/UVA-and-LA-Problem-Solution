...
int dp[1000][1000];
int play(string s, int i, int j){
    if(i>=j) return 0;
    if(dp[i][j] != -1) return dp[i][j];
    int mini = inf;
    if(s[i]==s[j]){
        mini = min(mini, play(s, i+1, j-1)); //same found
    }
    else{
        mini = min(mini, 1+play(s, i+1, j-1)); //replace a char and make it same (1 operation)
        mini = min(mini, 1+play(s, i, j-1)); //delete j th char (1 operation)
        mini = min(mini, 1+play(s, i+1, j)); //delete i th char (1 operation)
    }
    return dp[i][j]=mini;
}
