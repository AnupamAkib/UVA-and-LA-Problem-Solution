...
string dp_call(int pos_a, int pos_b){
    if(pos_b == b.sz) return "1";
    if(pos_a == a.sz) return "0";
    if(DP[pos_a][pos_b] != "-1") return DP[pos_a][pos_b];
    string ret = "0";
    if(a[pos_a] == b[pos_b]){
        ret = sum(ret, dp_call(pos_a+1, pos_b+1));
    }
    ret = sum(ret, dp_call(pos_a+1, pos_b));
    return DP[pos_a][pos_b]=ret;
}
