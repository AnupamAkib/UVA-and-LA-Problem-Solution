...
lli dp_call(int n, int idx){
    if(n<0 or idx==5) return 0;
    if(n==0) return 1;
    if(DP[n][idx] != -1) return DP[n][idx];
    return DP[n][idx] = dp_call(n-coin[idx], idx) + dp_call(n, idx+1);
}

void solve(int tc){
    lli ans = dp_call(tc, 0);
    if(ans==1){
        print("There is only "<< ans <<" way to produce "<< tc <<" cents change.")
    }
    else{
        print("There are "<< ans <<" ways to produce "<< tc <<" cents change.")
    }
}
