...
rat target, prevLeft, prevRight;
string res = "";

void BS(rat now){
    if(now.hor==target.hor and now.lob==target.lob){
        return;
    }
    if(now.lob/now.hor < target.lob/target.hor){ //go Right
        prevLeft.lob = now.lob, prevLeft.hor = now.hor;
        now.lob = prevLeft.lob + prevRight.lob;
        now.hor = prevLeft.hor + prevRight.hor;
        res += "R";
        BS(now);
    }
    else{ //go Left
        prevRight.lob = now.lob, prevRight.hor = now.hor;
        now.lob = prevLeft.lob + prevRight.lob;
        now.hor = prevLeft.hor + prevRight.hor;
        res += "L";
        BS(now);
...
void solve(int tc){
    rat now;
    res = "";
    if(target.lob/target.hor < 1){
        prevLeft.lob = 0, prevLeft.hor = 1;
        prevRight.lob = 1, prevRight.hor = 1;
        now.lob = prevLeft.lob + prevRight.lob;
        now.hor = prevLeft.hor + prevRight.hor;
        res += "L";
    }
    else{
        prevLeft.lob = 1, prevLeft.hor = 1;
        prevRight.lob = 1, prevRight.hor = 0;
        now.lob = prevLeft.lob + prevRight.lob;
        now.hor = prevLeft.hor + prevRight.hor;
        res += "R";
    }
...
//    }
    ...
    while(cin >> target.lob >> target.hor){
        if(target.lob==1 && target.hor==1) break;
        solve(1);
    }
