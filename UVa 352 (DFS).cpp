...
vector<pair<int, int> > getAdjacent(int i, int j){
    ...
    if(i-1>=0 and j-1>=0) adj.push_back({i-1, j-1});
    if(i-1>=0) adj.push_back({i-1, j});
    if(j-1>=0) adj.push_back({i, j-1});
    if(j+1<n) adj.push_back({i, j+1});
    if(i+1<n) adj.push_back({i+1, j});
    if(i+1<n and j+1<n) adj.push_back({i+1, j+1});
    if(i+1<n and j-1>=0) adj.push_back({i+1, j-1});
    if(i-1>=0 and j+1<n) adj.push_back({i-1, j+1});
    return adj;
}
...
void DFS(int i, int j){
    ...
    vis[i][j] = true;
    vector<pair<int, int> > allAdj = getAdjacent(i, j);
    for(auto p : allAdj){
        if(!vis[p.ff][p.ss]){
            DFS(p.ff, p.ss);
        }
...
void solve(lli tc){
    ...
    print("Image number " << t++ << " contains " << ans << " war eagles.")
}
